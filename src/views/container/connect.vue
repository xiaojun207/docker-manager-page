<template>
  <div class="app-container">
    ...
    />
    <el-input v-model="form.cmd" placeholder="请输入内容" style="width: 160px;margin-right: 10px;" />
    <el-button :loading="loading" class="filter-item" type="primary" icon="el-icon-search" :disabled="isConnected" @click="onStartExec">
      {{ $t('连接') }}
    </el-button>
    <el-button :loading="loading" class="filter-item" type="primary" :disabled="!isConnected" @click="onCloseExec">
      {{ $t('断开连接') }}
    </el-button>
    <div class="console">
      <terminal ref="terminal" @onInitTerm="isConnected = true" @onSocketClose="isConnected = false" @onSocketError="isConnected = false" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onStartExec() {
      ...
      const wsUri = WsHost() + '/dockerMgrApi/ws/exec?containerId=' + ContainerShortId(this.form.containerId) + '&cmd=' + this.form.cmd // ws地址
