...
export default {
  ...
  data() {
    return {
      roles: [{ 'label': '超级管理员', value: 'ADMIN' }, { 'label': '客户端代理', value: 'AGENT' }],
      formLoading: false,
      rules: {
        nickname: [{ required: true, message: '请输入昵称', trigger: 'blur' }, { min: 3, max: 15, message: '请输入正确的昵称', trigger: 'blur' }],
        username: [{ required: true, message: '请输入用户名', trigger: 'blur' }, { min: 3, max: 20, message: '请输入正确的用户名', trigger: 'blur' }],
        checkPhone: [{ required: true, validator: checkPhoneNumber, trigger: 'blur' }],
        checkId: [{ required: true, validator: checkIdNum, trigger: 'blur' }],
        mobile: [{ required: true, message: '请输入手机号码', trigger: 'blur' }, { min: 6, max: 11, message: '请输入正确的手机号码', trigger: 'blur' }, { pattern: /^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$/, message: '请输入正确的手机号码' }],
