  ...
  <div class="app-container">
    ...
    <el-form ref="form" :model="form" label-width="120px">
      ...
      <el-form-item :label="$t('卷映射')">
        ...
        <el-link type="info" :underline="false" href="https://docs.docker.com/storage/bind-mounts/#start-a-container-with-a-bind-mount" style="margin-left: 10px;">
      ...
      <el-form-item :label="$t('日志驱动')">
        ...
        <el-link type="info" :underline="false" href="https://docs.docker.com/config/containers/logging/configure/" style="margin-left: 10px;">
          {{ $t('如何选择日志驱动？') }}
        </el-link>
      </el-form-item>
    ...
    return {
      res: {
        ...
        logTypes: [
          ...
          {
            name: 'fluentd',
            summary: '将日志消息写入 fluentd（forward input）。需配置fluentd地址参数fluentd-address，或fluentd守护程序必须在主机上运行。'
          },
      ...
      tempPlaceholder: 'docker run -d --name test-openresty -p 80:80 -p 443:443 -m 512m -e user=root -v /tmp/openresty/nginx/logs:/usr/local/openresty/nginx/logs openresty/openresty:latest',
      formTmp: {
        Ports: [],
