<template>
  <div class="app-container">
    <el-form ref="form" :model="form" label-width="120px">
      <el-form-item :label="$t('Git URL')">
        <el-input
          v-model="form.sourceUrl"
          :placeholder="$t('请输入代码git URL')"
        />
      </el-form-item>
      ...
      <el-form-item :label="$t('Dockerfile位置')">
        <el-input
          ...
          :placeholder="$t('请输入Dockerfile位置')"
        />
        <div>{{ $t('打包命令') }}： docker build -t ${Image} -f ./Dockerfile .</div>
      </el-form-item>

      <el-form-item :label="$t('触发方式')">
        <el-radio v-model="form.webHook" label="2">{{
          $t('立即执行')
        }}</el-radio><br>
        <el-radio v-model="form.webHook" label="1">{{
          $t('推送到master时触发流水线')
        }}</el-radio>
        {{ $t('Webhook配置参数') }}：http://dockermanager.my, application/json, secret
      </el-form-item>

      <el-form-item :label="$t('镜像仓库地址')">
        <el-input
          v-model="form.imageUrl"
          :placeholder="$t('请输入镜像仓库地址URL')"
        />
        {{ $t('镜像仓库授权方式') }}
      </el-form-item>
