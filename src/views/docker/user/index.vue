  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('操作')" align="center">
        <template v-slot="scope" :loading="scope.row.Loading">
          ...
          <el-button
            ...
            type="primary"
            link
            @click="changeStatus(scope.row, scope.row.status === 0 ? 1 : 0)"
          >{{ scope.row.status === 0 ? $t('禁用') : $t('恢复') }}</el-button>
          <el-button
            size="small"
            type="primary"
            link
...
<script>
import { addUser, deleteUser, changeStatus, getUserList, resetPassword } from '@/api/user'
...
export default {
  ...
  data() {
    return {
      ...
      roles: [
        { label: this.$t('超级管理员'), value: 'ADMIN' },
        { label: this.$t('客户端代理'), value: 'AGENT' }
      ],
      formLoading: false,
      rules: {
        nickname: [
